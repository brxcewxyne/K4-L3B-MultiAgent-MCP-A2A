...
SEMANTIC_SYSTEM = (
    ...
    "the given known party IDs or null. Return only structured output as a "
    "single JSON object with EXACTLY these keys and no others: "
    ...
    "unsupported_claim, insufficient_evidence), secondary_issues (array of "
    "the same enum, max 10), case_status (action_required, no_action, "
    "needs_investigation), claim_assessments (one object per given claim: "
    "{claim_id, verdict} with verdict in supported, unsupported, "
    "partially_supported, insufficient_evidence), responsible_parties "
    "(1-5 objects: {party_type, party_id} with party_type in seller, "
    "platform, logistics_provider, payment_provider, customer, unknown and "
    "party_id a given known ID or null), ranked_causes (1-5 objects: "
    "{cause_code, rank} with cause_code the UPPER_SNAKE form of the primary "
    "or a secondary issue and rank an int 1-5, unique), "
    "resolution_action_codes (array of short semantic action code strings, "
    "max 8, e.g. issue_refund, refund_freight, reconcile_payment, "
    "retry_refund, monitor_refund, document_no_action), model_confidence "
    "(number 0-1)."
)
